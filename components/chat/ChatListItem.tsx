...
import { Check, CheckCheck, Globe, User, Users } from 'lucide-react';
...
export function ChatListItem({ conversation, isActive, onClick }: ChatListItemProps) {
  const getConversationIcon = () => {
    if (conversation.isGlobal) return <Globe className="w-4 h-4" />;
    if (conversation.type === 'GROUP') return <Users className="w-4 h-4" />;
  ...
  const getOnlineCount = () => conversation.participants.filter(p => p.isOnline).length;

  const getAvatarContent = () => {
    if (conversation.avatar) return <AvatarImage src={conversation.avatar} alt={getConversationName()} />;
    ...
    if (otherParticipant?.avatar) return <AvatarImage src={otherParticipant.avatar} alt={otherParticipant.name} />;
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        <Avatar className="w-12 h-12">{getAvatarContent()}</Avatar>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center space-x-2 min-w-0">
            <h3
              className={cn('font-medium truncate', isActive ? 'text-blue-900' : 'text-gray-900')}
            >
              {getConversationName()}
            </h3>
          ...
          <div className="flex items-center space-x-2 flex-shrink-0">
            ...
            {getLastMessageTime() && (
              <span className={cn('text-xs', isActive ? 'text-blue-600' : 'text-gray-500')}>
                {getLastMessageTime()}
              </span>
            )}
          </div>
        ...
        <div className="flex items-center justify-between">
          <p className={cn('text-sm truncate', isActive ? 'text-blue-700' : 'text-gray-600')}>
