import type { Conversation, Message, SearchResult, User } from '@/types/chat';
...
export const useChatStore = create<ChatState & ChatActions>()(
  persist(
    (set, get) => ({
      ...
      logout: () => set({ ...initialState, conversations: [], messages: {}, activeConversationId: null }),
      ...
      updateMessage: (conversationId, messageId, updates) =>
        set((state) => {
          ...
          return {
            messages: {
              ...state.messages,
              [conversationId]: currentMessages.map((msg) => (msg.id === messageId ? { ...msg, ...updates } : msg)),
            },
          };
        }),
      prependMessages: (conversationId, messages) =>
        set((state) => {
          const currentMessages = state.messages[conversationId] || [];
          return { messages: { ...state.messages, [conversationId]: [...messages, ...currentMessages] } };
        }),
      ...
      addTypingUser: (conversationId, userId) =>
        set((state) => {
          ...
          if (!currentTyping.includes(userId)) {
            return { typingUsers: { ...state.typingUsers, [conversationId]: [...currentTyping, userId] } };
          }
          return state;
        }),
      removeTypingUser: (conversationId, userId) =>
        set((state) => {
          const currentTyping = state.typingUsers[conversationId] || [];
          return { typingUsers: { ...state.typingUsers, [conversationId]: currentTyping.filter((id) => id !== userId) } };
        }),
...
export const useMessages = (conversationId: string) => useChatStore((state) => state.messages[conversationId] || []);
export const useTypingUsers = (conversationId: string) => useChatStore((state) => state.typingUsers[conversationId] || []);
